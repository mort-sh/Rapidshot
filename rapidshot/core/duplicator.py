...
class Duplicator:
    ...
    def __post_init__(self, output: Output, device: Device) -> None:
        ...
        try:
            ...
            if _hresult_eq(hresult, DXGI_ERROR_INVALID_CALL) or _hresult_eq(
                hresult, DXGI_ERROR_UNSUPPORTED
            ):
                raise RapidShotConfigError(
                    f"Failed to initialize duplicator due to configuration or invalid call: {ce}",
                    hresult=hresult,
                ) from ce
            elif _hresult_eq(hresult, DXGI_ERROR_DEVICE_REMOVED) or _hresult_eq(
                hresult, DXGI_ERROR_DEVICE_RESET
            ):
                raise RapidShotDeviceError(
                    f"Failed to initialize duplicator due to device error: {ce}",
                    hresult=hresult,
                ) from ce
            else:
                raise RapidShotDXGIError(
                    f"Failed to initialize duplicator: {ce}", hresult=hresult
                ) from ce
    ...
    def update_frame(self) -> None:
        ...
        try:
            ...
            hresult = ce.args[0] if ce.args else None
            self.last_error = (
                f"COMError in update_frame: {ce} (HRESULT: {_format_hresult(hresult)})"
            )
            ...
            if _hresult_eq(hresult, DXGI_ERROR_WAIT_TIMEOUT):
            ...
            elif _hresult_eq(hresult, DXGI_ERROR_ACCESS_LOST) or _hresult_eq(
                hresult, ABANDONED_MUTEX_EXCEPTION
            ):
                ...
                raise RapidShotReinitError(
                    f"Access lost, re-initialization needed: {ce}", hresult=hresult
                ) from ce
            elif _hresult_eq(hresult, DXGI_ERROR_DEVICE_REMOVED) or _hresult_eq(
                hresult, DXGI_ERROR_DEVICE_RESET
            ):
                ...
                # Other COM errors
                raise RapidShotDXGIError(
                    f"Unexpected DXGI error in update_frame: {ce}", hresult=hresult
                ) from ce
            ...
            raise RapidShotError(
                f"Unhandled Python exception in update_frame: {e}"
            ) from e  # Wrap in RapidShotError
        ...
        if self.duplicator is not None:
            try:
                ...
                if ce.args and _hresult_eq(ce.args[0], DXGI_ERROR_INVALID_CALL):
                    ...
                    logger.warning(
                        f"Failed to release frame: {ce} (HRESULT: {hresult_text})"
                    )
    ...
    def get_frame_pointer_shape(
        self, frame_info
    ) -> Union[
        Tuple[DXGI_OUTDUPL_POINTER_SHAPE_INFO, bytes, str], Tuple[bool, bool, str]
    ]:
        """
        ...
        try:
            ...
            if hr >= 0:  # Success
                logger.debug(
                    f"Cursor shape acquired: {pointer_shape_info.Width}x{pointer_shape_info.Height}, Type: {pointer_shape_info.Type}"
                )
            ...
            if _hresult_eq(hresult, DXGI_ERROR_ACCESS_LOST):
                ...
                return (
                    False,
                    False,
                    f"Cursor shape not found (HRESULT: {hresult:#010x})",
                )
    ...
    def __repr__(self) -> str:
        ...
        return "<{} Initialized:{} Cursor:{}>".format(
            self.__class__.__name__, self.duplicator is not None, cursor_status
        )
