...
class Device:
    ...
    def _create_device_with_multiple_fallbacks(self) -> None:
        ...
        for level in feature_levels:
            if hasattr(
                ...
                f"D3D_FEATURE_LEVEL_{level >> 12}_{(level >> 8) & 0xF}",
            ):
        ...
        logger.debug(
            f"Using feature levels: {[hex(level) for level in available_feature_levels]}"
        )

        # Convert feature levels to C array
        feature_levels_array = (ctypes.c_uint * len(available_feature_levels))(
            *available_feature_levels
        )
                ...
                try:
                    ...
                    if driver_type == D3D_DRIVER_TYPE_UNKNOWN and adapter_ptr:
                        ...
                        if not bool(adapter_ptr):
                            logger.warning(
                                "Adapter pointer is null, skipping this attempt"
                            )
                            continue
                        ...
                        if result != 0:  # non-zero = error
                            logger.debug(
                                f"D3D11CreateDevice returned error code: {result:#x}"
                            )
                            # Convert to COMError to be caught below
                            raise comtypes.COMError(
                                result,
                                None,
                                f"D3D11CreateDevice failed with code {result:#x}",
                            )
                    else:
                        # Use driver type
                        result = _D3D11CreateDevice(
                    ...
                    if not device_ptr.value or not context_ptr.value:
                        logger.warning(
                            "Device or context pointer is null after D3D11CreateDevice"
                        )
                        continue

                    # Convert void pointers to the correct interface types
                    self.device = ctypes.cast(device_ptr, ctypes.POINTER(ID3D11Device))
                    self.context = ctypes.cast(
                        context_ptr, ctypes.POINTER(ID3D11DeviceContext)
                    )
                    ...
                    # Log success
                    logger.info(
                        f"Successfully created device with feature level {self.feature_level_to_str(self.feature_level)}"
                    )
                    created = True
                    break
                    ...
                    last_error = ce
                    logger.debug(
                        f"Failed to create device with driver {driver_type}, flags {flags}: {ce}"
                    )
                    continue
    ...
    def create(cls, adapter_idx=0):
        ...
        if adapter_idx >= len(adapters):
            logger.error(
                f"Adapter index {adapter_idx} out of range, found {len(adapters)} adapters"
            )
            raise IndexError(
                f"Adapter index {adapter_idx} out of range, found {len(adapters)} adapters"
            )
