...
class Processor:
    ...
    def process(
        ...
        return self.backend.process(
            rect, width, height, region, rotation_angle, output_buffer
        )
    ...
    def process2(self, image_ptr, rect, width, height):
        ...
        if hasattr(self.backend, "shot"):
            ...
            raise NotImplementedError(
                "Direct buffer processing not supported by this backend"
            )
    ...
    def _initialize_backend(self, backend: ProcessorBackends):
        ...
        if backend == ProcessorBackends.PIL:
            try:
                ...
                raise ImportError(
                    "No available backend. Please install either NumPy or PIL."
                )
